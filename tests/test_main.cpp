...
static void test_system_info() {
    ...
    // git_commit is regenerated by CMake at every configure (see
    // build_info.hpp.in) from the tree actually being built -- "unknown"
    // is a legitimate value (a source tarball with no .git directory),
    // but an empty string would mean the substitution silently failed.
    CHECK(!info.git_commit.empty());
    std::printf("  (system: %s, %s (%s), %u logical cores, SIMD=%s, RAM=%s, CUDA=%s, commit=%s%s)\n",
                 ...
                 info.git_commit.c_str(), info.git_dirty ? " (dirty)" : "");
}
